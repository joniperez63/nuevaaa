...
const GiverForm: React.FC<GiverFormProps> = ({ onSubmit, onBack }) => {
  ...
  const handleFormSubmit = (e: React.FormEvent, addAnother: boolean) => {
    e.preventDefault();
    ...
    // Convert months to years for storage if needed
    if (ageUnit === 'months') {
    ...
    const newPet: Pet = {
      ...
      age: numericAge,
      breed: breed || 'Mestizo',
      image: images[0], // Using the first image as main
      bio,
      personality: 'Friendly, waiting for a home.',
      distance: 0, // It's the user's pet
      tags
    };

    onSubmit(newPet, addAnother);
    ...
    <div className="h-full bg-white flex flex-col">
      ...
      <div className="p-6 border-t border-gray-100 flex flex-col gap-3">
        <button 
          ...
          className="w-full py-3 bg-white border-2 border-teal-500 text-teal-500 font-bold rounded-full shadow-sm hover:bg-teal-50 transition flex items-center justify-center gap-2"
        >
          <PlusCircle size={20} />
        ...
        <button 
          ...
          className="w-full py-4 bg-teal-500 text-white font-bold rounded-full shadow-lg hover:bg-teal-600 transition"
        >
